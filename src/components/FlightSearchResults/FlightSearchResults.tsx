...
const FlightSearchResults: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        // Split round-trip flights into departure and return
        const splitFlights: Flight[] = [];
        flightsArr.forEach((flight) => {
          if (flight.trips.length === 2) {
            splitFlights.push({
              ...flight,
              trips: [flight.trips[0]], // Departure flight
              totalPrice: (parseFloat(flight.totalPrice) / 2).toFixed(2),
              basePrice: (parseFloat(flight.basePrice) / 2).toFixed(2),
              oneWay: true,
            });
            splitFlights.push({
              ...flight,
              trips: [flight.trips[1]], // Return flight
              totalPrice: (parseFloat(flight.totalPrice) / 2).toFixed(2),
              basePrice: (parseFloat(flight.basePrice) / 2).toFixed(2),
              oneWay: true,
            });
          } else {
            splitFlights.push(flight);
          }
        });
        setFlights(splitFlights);
        setFilteredFlights(splitFlights);
        const prices = splitFlights.map((f) => parseFloat(f.totalPrice || f.basePrice || "0") || 0);
        setMinPrice(Math.min(...prices));
        setMaxPrice(Math.max(...prices));
        const stopsSet = new Set<string>(),
          airlinesSet = new Set<string>();
        splitFlights.forEach((f) => {
  ...
  // Compute count for flights
  const count = currentStep === 'departure'
    ? filteredFlights.filter((f) => f.trips[0]?.from === from && f.trips[0]?.to === to).length
    : filteredFlights.filter((f) => f.trips[0]?.from === to && f.trips[0]?.to === from).length;
      ...
      >
        <Box display="flex" alignItems="center" gap={1.2}>
          ...
          <Typography fontWeight={700}>
            {currentStep === "departure" ? `${count} departure flights found` : `Recommended returning flights (${count})`}
          </Typography>
          <Chip
            label={`${count}`}
            size="small"
  ...
  // Combine departure and return flights for navigation
  const combinedFlight: Flight | null = selectedDepartureFlight && selectedReturnFlight ? {
    ...selectedDepartureFlight,
    trips: [selectedDepartureFlight.trips[0], selectedReturnFlight.trips[0]],
    totalPrice: (
      (parseFloat(selectedDepartureFlight.totalPrice) || 0) +
      (parseFloat(selectedReturnFlight.totalPrice) || 0)
    ).toFixed(2),
    basePrice: (
      (parseFloat(selectedDepartureFlight.basePrice) || 0) +
      (parseFloat(selectedReturnFlight.basePrice) || 0)
    ).toFixed(2),
    currencyCode: selectedDepartureFlight.currencyCode,
    oneWay: false,
    seatsAvailable: Math.min(
      selectedDepartureFlight.seatsAvailable || 0,
      selectedReturnFlight.seatsAvailable || 0
    ),
  } : null;
